...
function Form({ type }) {
    ...
    const submit = async (data) => {
        ...
        if (type == "login") {
            let res = await axios.post('https://blogapplicatonfrontend.onrender.com/auth/login', data)
        ...
        if (type === "register") {
            try {
                let res = await axios.post('https://blogapplicatonfrontend.onrender.com/auth/register', data)
                console.log(res);
                if (res?.data?.status === 200) {
                    toast.success(res?.data?.message)
                    nevigate('/auth/login')
